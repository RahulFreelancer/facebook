...
const UniversityForm= ({changeShowUniversityForm}) => {
const {myAudience,updateMyAudience,about,updateAboutProperty}=useContext(LightStoreContext);
const {user}=useContext(UserContext);


const [form,changeForm]=useState({name:'',course:'',degree:'',audience:'',graduated:''});
const [isGraduated,changeIsGraduated]=useState(true);
useEffect(()=>{
  
        if(about.workAndEdu&&about.workAndEdu.university&&about.workAndEdu.university.audience){
            updateMyAudience(about.workAndEdu.university.audience);
        }
        else(updateMyAudience('public'))
 
    if(about.workAndEdu&&about.workAndEdu.university&&about.workAndEdu.university.name){
        const data = {...form};
     data.name=about.workAndEdu.university.name;
     data.course=about.workAndEdu.university.course;
     data.degree=about.workAndEdu.university.degree;
     data.graduated=about.workAndEdu.university.graduated;
     changeForm(data);
    }
...
changeForm(data);
    ...
    <form className={styles.workForm}>
        <input type='text' name='name' onChange={handleFormChange} value={form.name} className={styles.textInput} placeholder='Name of Institute'>
        </input>
        <input type='text' name='degree' onChange={handleFormChange} value={form.degree} className={styles.textInput}placeholder='Degree'>
        </input>
        <input type='text' name='course' onChange={handleFormChange} value = {form.course}className={styles.textInput} placeholder='Course'>
        </input>
  ...
  <strong>Graduated</strong>
       ...
       <div className={styles.saveAndCanelButtonBox}>
        ...
        <button disabled={form.name&&form.course&&form.degree?false:true}
        onClick={saveForm} 
